...
首页“AI 活动预筛选”当前已经支持真实大模型调用，后端调用的是兼容 OpenAI 风格的：
...
```bash
export CAMPUSFLOW_LLM_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
export CAMPUSFLOW_LLM_API_KEY=sk-a917435994694963a379191ed4a993ba
export CAMPUSFLOW_LLM_MODEL=qwen-plus
```

然后启动后端：

```bash
cd /home/rising5/Software_lab/CampusFlow-lab4/CampusFlow-lab4/backend
python3 app.py
```

你也可以直接在首页的“真实模型接口预留”区域填写：

- `API Base URL`
- `Model`
- `API Key`

这三个字段会随请求发到后端的 `/api/ai/activity-screening`，由后端再转发到真实模型接口。

如果你要换成自己的平台，主要改这里：

- 后端调用入口：[backend/campusflow_py/server.py](/home/rising5/Software_lab/CampusFlow-lab4/CampusFlow-lab4/backend/campusflow_py/server.py)
  - `resolve_llm_config`
  - `screen_activities_via_llm`
  - `call_openai_compatible_api`

默认行为：

- 配了完整 `Base URL + API Key + Model`：优先走真实模型
- 调用失败：自动回退到本地启发式分析
- 没配置：直接走本地启发式分析
