...
class CampusFlowStore:
    ...
    def screen_activities_with_ai(self, payload: dict[str, Any]) -> dict[str, Any]:
        prompt = str(payload.get("prompt") or "").strip()
        attachment_name = str(payload.get("attachmentName") or "").strip()
        attachment_text = str(payload.get("attachmentText") or "").strip()
        require_team = _bool(payload.get("requireTeam"), None)
        config = self.resolve_llm_config(payload)
        if not prompt and not attachment_text and require_team is None:
            raise BusinessError("请至少输入活动诉求、上传材料，或指定参与方式")

        heuristic = self.screen_activities_heuristic(prompt, attachment_name, attachment_text, require_team, config)
        if not config["configured"]:
            return heuristic
        try:
            llm_result = self.screen_activities_via_llm(prompt, attachment_name, attachment_text, require_team, config)
            return self.merge_ai_result(llm_result, heuristic, config, "llm_live", "已调用真实大模型接口并返回推荐结果。")
            ...
            fallback["configStatus"] = {
                ...
                "configured": True,
                "mode": "llm_fallback",
                "note": f"真实模型调用失败，已回退到本地规则分析。失败原因：{exc}",
            }
            fallback["suggestedActions"] = [
                f"检查模型接口是否可达：{config['endpoint'] or '未配置 endpoint'}",
                "确认 API Key、Model 名称以及网关是否兼容 chat/completions。",
                *fallback.get("suggestedActions", [])[:2],
            ]
            return fallback

    def resolve_llm_config(self, payload: dict[str, Any]) -> dict[str, Any]:
        endpoint = str(
            payload.get("apiBaseUrl")
            or os.environ.get("CAMPUSFLOW_LLM_BASE_URL")
            or os.environ.get("OPENAI_BASE_URL")
            or ""
        ).strip()
        api_key = str(
            payload.get("apiKey")
            or os.environ.get("CAMPUSFLOW_LLM_API_KEY")
            or os.environ.get("OPENAI_API_KEY")
            or ""
        ).strip()
        model = str(
            payload.get("model")
            or os.environ.get("CAMPUSFLOW_LLM_MODEL")
    ...
    def screen_activities_heuristic(
        self,
        prompt: str,
        attachment_name: str,
        attachment_text: str,
        require_team: bool | None,
        config: dict[str, Any],
    ) -> dict[str, Any]:
        source_text = "\n".join(part for part in [prompt, attachment_name, attachment_text[:4000]] if part).lower()
        focus_pool = [
            ("技术实践", ["技术", "ai", "算法", "编程", "原型", "产品", "数据", "开发", "机器人"]),
            ("创新竞赛", ["创新", "创业", "竞赛", "路演", "作品集", "方案"]),
            ("公益服务", ["公益", "志愿", "支教", "低碳", "服务"]),
            ("文化体验", ["文化", "摄影", "手作", "阅读", "音乐", "辩论", "影像"]),
            ("成长支持", ["成长", "心理", "同伴", "训练", "学习"]),
            ("体育活动", ["体育", "运动", "嘉年华"]),
            ("近期可参加", ["近期", "尽快", "马上", "本周", "下周", "最近", "5月", "五月"]),
        ]
        extracted_context: list[str] = []
        for label, keywords in focus_pool:
            if any(keyword in source_text for keyword in keywords):
                extracted_context.append(label)
        if "组队" in source_text or "团队" in source_text:
            extracted_context.append("偏好组队")
        if "个人" in source_text or "solo" in source_text:
            extracted_context.append("偏好个人参与")
        if not extracted_context and prompt:
            extracted_context.append("根据自由描述做匹配")
        if attachment_name:
            extracted_context.append(f"已分析附件：{attachment_name}")
        ...
        for activity in self.activities:
            ...
            score = 20 if activity["status"] == "signup_open" else 8
            reasons: list[str] = []
            highlights: list[str] = []
            if activity["status"] == "signup_open":
                reasons.append("当前仍在开放报名")
                highlights.append("可直接进入报名")
            if require_team is not None:
                if activity["requireTeam"] == require_team:
                    score += 12
                    reasons.append("参与方式与当前偏好一致")
                else:
                    score -= 8
            if activity["requireTeam"] and any(tag in extracted_context for tag in ["技术实践", "创新竞赛", "公益服务", "体育活动"]):
                score += 5
            if (not activity["requireTeam"]) and any(tag in extracted_context for tag in ["文化体验", "成长支持"]):
                score += 5
            for label, keywords in focus_pool:
                matched = [keyword for keyword in keywords if keyword in source_text and keyword in haystack]
                if matched:
                    score += 9 + min(len(matched), 2) * 2
                    reasons.append(f"和“{label}”诉求匹配")
                    highlights.extend(matched[:2])
            title_type_matches = [term for term in _tags(prompt) if term and term.lower() in haystack]
            if title_type_matches:
                score += min(len(title_type_matches), 3) * 3
                highlights.extend(title_type_matches[:2])
            start_time = _dt(activity.get("startTime"))
            if start_time is not None:
                days_until = (start_time - _now()).days
                if 0 <= days_until <= 10:
                    score += 6
                    reasons.append("活动时间较近，适合快速推进")
            if "作品集" in source_text and activity["type"] in {"技术沙龙", "技术竞赛", "创新竞赛"}:
                score += 8
                reasons.append("更适合沉淀作品或项目经历")
                highlights.append("作品集友好")
            if "志愿" in source_text and "志愿" in haystack:
                score += 8
            if "摄影" in source_text and "摄影" in haystack:
                score += 8
            if not reasons:
                reasons.append("活动主题与当前描述存在基础匹配")
            deduped_highlights: list[str] = []
            for item in highlights:
                normalized = str(item).strip()
                if normalized and normalized not in deduped_highlights:
                    deduped_highlights.append(normalized)
            recommendations.append(
                {
                    ...
                    "matchHighlights": deduped_highlights[:4] or list(activity.get("tags") or [])[:2],
                    "nextStep": "进入活动详情后继续个人报名" if not activity["requireTeam"] else "进入活动详情后创建队伍或申请加入现有队伍",
                }
            )
        recommendations.sort(key=lambda item: (-item["score"], item["activityId"]))
        top_results = recommendations[:4]
        if not top_results:
            return {
                "summary": "当前没有符合条件的活动，建议先放宽参与方式或活动类型约束。",
                "focusTags": ["建议放宽条件"],
                "extractedContext": extracted_context or ["未识别到稳定偏好"],
                "suggestedActions": ["将参与方式改为不限", "先浏览全部开放报名活动", "再根据详情页决定是否组队"],
                ...
                "configStatus": {
                    ...
                    "mode": "heuristic_demo",
                    "note": "当前返回的是本地启发式分析结果。",
                },
            }
        top_titles = "、".join(item["title"] for item in top_results[:3])
        focus_tags: list[str] = []
        for item in top_results:
            if item["type"] not in focus_tags:
                focus_tags.append(item["type"])
            if item["requireTeam"] and "组队协作" not in focus_tags:
                focus_tags.append("组队协作")
            if (not item["requireTeam"]) and "个人直报" not in focus_tags:
                focus_tags.append("个人直报")
        return {
            "summary": f"综合你的描述，当前最值得优先看的活动是 {top_titles}。排序优先考虑了报名开放状态、参与方式匹配度和文本语义线索。",
            "focusTags": focus_tags[:6],
            "extractedContext": extracted_context[:6] or ["未识别到明确偏好，已按开放报名优先排序"],
            "suggestedActions": [
                "先点进得分最高的活动详情，确认时间、地点和参与规则。",
                "若推荐结果偏团队型，优先检查队伍人数门槛和当前可加入队伍。",
                "若已配置真实模型接口，可直接在这个入口切到真实模型分析。",
            ],
            "recommendedActivities": top_results,
            "configStatus": {
                "provider": config["provider"],
                "endpoint": config["endpoint"],
                "model": config["model"],
                "configured": config["configured"],
                "mode": "heuristic_demo",
                "note": "当前未配置完整模型接口，先使用本地启发式规则做演示分析。"
                if not config["configured"]
                else "真实模型可用，但当前结果来自本地规则分析。",
            },
        }

    def screen_activities_via_llm(
        self,
        prompt: str,
        attachment_name: str,
        attachment_text: str,
        ...
        endpoint = str(config["endpoint"]).rstrip("/") + "/chat/completions"
        activity_catalog = []
        for activity in self.activities:
            if activity["status"] == "draft":
                continue
            activity_catalog.append(
                {
                    ...
                    "startTime": activity["startTime"],
                    "tags": list(activity.get("tags") or []),
                    "description": str(activity.get("description") or "")[:220],
                }
            )
        system_prompt = (
            "你是校园活动匹配助手。请只输出 JSON 对象，不要输出 markdown。"
            "返回字段必须包含 summary、focusTags、extractedContext、suggestedActions、recommendedActivities。"
            "recommendedActivities 是数组，每项包含 activityId、score、reason、matchHighlights、nextStep。"
            "score 范围 1-100，只能推荐给定活动列表中的 activityId。"
        )
        user_payload = {
            "userPrompt": prompt,
            "attachmentName": attachment_name or None,
            "attachmentTextPreview": attachment_text[:5000] if attachment_text else None,
            "requireTeamPreference": require_team,
            "activities": activity_catalog[:18],
        }
        request_body = {
            "model": config["model"],
            "temperature": 0.2,
        ...
        raw = self.call_openai_compatible_api(endpoint, str(config["apiKey"]), request_body)
        content = self.extract_llm_content(raw)
        parsed = _first_json_object(content)
        if parsed is None:
            raise BusinessError("模型返回内容不是合法 JSON")
        return self.normalize_llm_result(parsed)

    def call_openai_compatible_api(self, endpoint: str, api_key: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            with urlopen(request, timeout=DEFAULT_LLM_TIMEOUT) as response:
                charset = response.headers.get_content_charset("utf-8")
                text = response.read().decode(charset)
        except HTTPError as exc:
            error_text = exc.read().decode("utf-8", errors="ignore")
            raise BusinessError(f"模型接口 HTTP {exc.code}：{error_text[:220] or exc.reason}") from exc
        except URLError as exc:
            raise BusinessError(f"模型接口连接失败：{exc.reason}") from exc
        try:
            data = json.loads(text)
        except Exception as exc:
            raise BusinessError(f"模型接口返回了非 JSON 内容：{text[:220]}") from exc
        if not isinstance(data, dict):
            raise BusinessError("模型接口返回结构不正确")
        return data

    def extract_llm_content(self, response_json: dict[str, Any]) -> str:
        choices = response_json.get("choices")
        if not isinstance(choices, list) or not choices:
            raise BusinessError("模型接口未返回 choices")
        message = choices[0].get("message") if isinstance(choices[0], dict) else None
        if not isinstance(message, dict):
            raise BusinessError("模型接口返回 message 结构异常")
        ...
        if isinstance(content, list):
            ...
            if parts:
                return "\n".join(parts)
        raise BusinessError("模型接口未返回可解析的 content")

    def normalize_llm_result(self, raw: dict[str, Any]) -> dict[str, Any]:
        recommendations = raw.get("recommendedActivities")
        if not isinstance(recommendations, list):
            raise BusinessError("模型返回缺少 recommendedActivities")
        normalized_items = []
        for item in recommendations[:6]:
            if not isinstance(item, dict):
                continue
            activity = self.activity_by_id(_int(item.get("activityId")))
            ...
            normalized_items.append(
                {
                    ...
                    "requireTeam": activity["requireTeam"],
                    "status": activity["status"],
                    "score": max(min(_int(item.get("score"), 75) or 75, 100), 1),
                    "reason": str(item.get("reason") or "模型认为该活动匹配当前诉求").strip(),
                    "matchHighlights": _tags(item.get("matchHighlights"))[:4] or list(activity.get("tags") or [])[:2],
                    "nextStep": str(item.get("nextStep") or "").strip()
                    or ("进入活动详情后继续个人报名" if not activity["requireTeam"] else "进入活动详情后创建队伍或申请加入现有队伍"),
                }
            )
        if not normalized_items:
            raise BusinessError("模型未返回有效推荐活动")
        normalized_items.sort(key=lambda item: (-item["score"], item["activityId"]))
        return {
            "summary": str(raw.get("summary") or "模型已完成活动匹配分析。").strip(),
            "focusTags": _tags(raw.get("focusTags"))[:6],
            "extractedContext": _tags(raw.get("extractedContext"))[:6],
            "suggestedActions": _tags(raw.get("suggestedActions"))[:6],
            "recommendedActivities": normalized_items[:4],
        }
    ...
    def merge_ai_result(
        ...
        if not result.get("focusTags"):
            result["focusTags"] = heuristic.get("focusTags") or []
        if not result.get("extractedContext"):
            result["extractedContext"] = heuristic.get("extractedContext") or []
        if not result.get("suggestedActions"):
            result["suggestedActions"] = heuristic.get("suggestedActions") or []
        result["configStatus"] = {
            "provider": config["provider"],
            "endpoint": config["endpoint"],
            "model": config["model"],
...
class CampusFlowHandler(BaseHTTPRequestHandler):
    ...
    def dispatch(
        ...
        if method == "POST" and path == "/api/ai/activity-screening":
            return "分析成功", store.screen_activities_with_ai(body)
