  ...
  <div class="list-stack" v-if="detail">
        ...
        <div class="hero-inner">
          <div class="hero-copy">
            ...
            <div class="hero-journey">
              <div v-for="item in journeySteps" :key="item.title" class="journey-chip">
                <span>{{ item.step }}</span>
    ...
    <div class="card-grid two-col">
        ...
        <article class="glass-card content-panel">
          ...
          <div class="insight-strip">
            <div class="insight-card">
              <span>报名判断</span>
              <strong>{{ detail.requireTeam ? '需先完成队伍协作' : '可直接个人报名' }}</strong>
            </div>
            <div class="insight-card">
              <span>审核后动作</span>
              <strong>{{ detail.requireTeam ? '队伍成员统一进入签到' : '个人审核通过后进入签到' }}</strong>
            </div>
            <div class="insight-card">
              <span>闭环终点</span>
              <strong>活动结束后提交反馈与复盘</strong>
            </div>
          </div>
      ...
      <aside class="list-stack">
        ...
        <article class="glass-card side-info">
          <h3 class="section-title">流程提醒</h3>
          <div class="timeline-list compact-list">
            <div class="timeline-item">
              <span>1. 先确认规则</span>
              <strong>{{ detail.requireTeam ? '先看人数门槛和组队要求' : '先看截止时间和报名说明' }}</strong>
            </div>
            <div class="timeline-item">
              <span>2. 再进入报名</span>
              <strong>{{ detail.requireTeam ? '创建队伍或申请加入已有队伍' : '填写报名说明后提交审核' }}</strong>
            </div>
            <div class="timeline-item">
              <span>3. 审核通过后</span>
              <strong>按通知进入签到，活动结束后提交反馈</strong>
            </div>
          </div>
        </article>
      </aside>
...
const primaryActionHint = computed(() => {
  if (!detail.value) return ''
  if (!userStore.isLoggedIn) return '先登录，再由系统判断你当前可以直接报名、组队还是签到。'
  if (detail.value.canSignupPersonal) return '这是个人活动，可以直接提交报名说明进入审核。'
  if (detail.value.canCreateTeam) return '这是团队活动，优先创建队伍并补齐成员信息。'
  if (detail.value.canApplyTeam) return '当前适合先查找队伍并提交入队申请。'
  if (detail.value.myTeamId) return '你已经在队伍里，直接进入队伍页继续提交报名。'
  if (detail.value.canSignIn) return '报名已通过，下一步是按时间窗口进入签到。'
  if (detail.value.canFeedback) return '活动已结束且你已签到，可以提交反馈完成闭环。'
  return '当前以查看状态为主，等待审核结果或活动时间推进。'
})
const journeySteps = computed(() => [
  { step: '01', title: '判断是否适合' },
  { step: '02', title: detail.value?.requireTeam ? '组队或入队' : '个人报名' },
  { step: '03', title: '审核与通知' },
  { step: '04', title: '签到与反馈' }
])
...
.journey-chip {
  padding: 14px 16px;
  border-radius: 18px;
  background: color-mix(in srgb, var(--cf-hero-soft-overlay) 92%, transparent);
